...
export const TraineeDetail: React.FC<TraineeDetailProps> = ({
    ...
    const handleReopenAll = async () => {
        const scopeLabel = targetScope === 'all' ? 'ALL' : (targetScope === '2D_Drawing' ? '2D Detailing' : '3D Modeling');
        const confirmed = await requestConfirmation({
        ...
        <section className="trainee-detail-view" style={{ height: '100%' }}>
            ...
            <div className="detail-container" style={{ flex: 1, minHeight: 0, alignItems: 'stretch' }}>
                ...
                <div className="detail-main" style={{ display: 'flex', flexDirection: 'column', minHeight: 0 }}>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                                ...
                                {(activeCourseTab === 'all' || activeCourseTab === '3D') && (
                                    <div className="course-tabs" style={{ display: 'flex', gap: '10px' }}>
                                        ...
                                        <button className={`sub-tab-btn ${(activeCourseTab as string) === '2D' ? 'active' : ''}`} onClick={() => setActiveCourseTab('2D')}>2D Detailing</button>
                                    </div>
                                )}
                            </div>
                    ...
                    {/* Assessment Matrix - 2D Detailing */}
                    {(activeCourseTab === 'all' || activeCourseTab === '2D') && (
                        <div className="history-section matrix-section" style={{ flex: '0 1 auto', display: 'flex', flexDirection: 'column', minHeight: 0, paddingBottom: 0 }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                                <h3 style={{ margin: 0, padding: 0, borderBottom: 'none', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                    <FileText size={18} /> ICAD OPERATION MANUAL 2D Detailing
                                    <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)', fontWeight: 500, marginLeft: '0.25rem' }}>
                                ...
                                {activeCourseTab === '2D' && (
                                    <div className="course-tabs" style={{ display: 'flex', gap: '10px' }}>
                                        ...
                                        <button className={`sub-tab-btn ${activeCourseTab === '2D' ? 'active' : ''}`} onClick={() => setActiveCourseTab('2D')}>2D Detailing</button>
                                    </div>
                                )}
                            </div>
                            {selectedTrainee.quizzes_history.filter(q => q.course_id === '2').length > 0 ? (
                                ...
                                <div className="empty-state" style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '1.5rem' }}>No 2D Detailing assessments recorded.</div>
                            )}
                        </div>
                    )}
                </div>
