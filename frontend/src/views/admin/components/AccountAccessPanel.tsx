...
export const AccountAccessPanel: React.FC<Props> = ({ user, onClose, onSaved }) => {
    ...
    React.useEffect(() => { Promise.all([adminService.getUserAccess(user.id), adminService.getUserPermissions(user.id)]).then(([access, permissionState]) => { setForm(access); setPermissions(permissionState.permissions); }).catch(() => setError('Unable to load account access.')).finally(() => setLoading(false)); }, [user.id]);
    ...
    const togglePermission = (code: string) => setPermissions(current => current.map(item => item.code === code ? { ...item, enabled: !item.enabled } : item));
    const submit = async (event: React.FormEvent) => {
        ...
        try {
            ...
            if (updated.role_code === 'admin') {
                const fresh = await adminService.getUserPermissions(user.id);
    ...
    const savePermissions = async () => {
        ...
        try {
            const enabled_codes = permissions.filter(item => item.enabled).map(item => item.code);
            const saved = await adminService.updateUserPermissions(user.id, { enabled_codes, reason: reason.trim(), reauth_password: reauthPassword });
            setPermissions(saved.permissions); setReason(''); setReauthPassword(''); await onSaved();
        } catch (err: any) { setError(err?.response?.data?.detail || 'Permissions could not be updated.'); }
        finally { setSaving(false); }
    };
    return <div className="account-access-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="account-access-panel" role="dialog" aria-modal="true">
        ...
        {loading || !form ? <div className="account-access-loading"><Loader2 className="spin" /> Loading access…</div> : <form onSubmit={submit}>
            ...
            {form.role_code === 'admin' && permissions.length > 0 && <section><h3>Granular permissions</h3><p>Fine-tune actions within the currently saved Admin Panel areas. Save area changes first when adding or removing an area.</p><div className="account-permission-list">{permissions.map(item => <label key={item.code}><input type="checkbox" checked={item.enabled} onChange={() => togglePermission(item.code)} /><span><strong>{item.code.split('.').map(word => word[0].toUpperCase() + word.slice(1)).join(' ')}</strong><small>{AREA_LABELS[item.area]} · {item.description}</small></span></label>)}</div><button type="button" className="account-permission-save" disabled={saving || reason.trim().length < 3 || !reauthPassword} onClick={savePermissions}><ShieldCheck size={16} /> Save permissions</button></section>}
