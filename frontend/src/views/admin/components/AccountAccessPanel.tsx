...
export const AccountAccessPanel: React.FC<Props> = ({ user, onClose, onSaved }) => {
    ...
    const submit = async (event: React.FormEvent) => {
        ...
        try {
            const updated = await adminService.updateUserAccess(user.id, { role_code: form.role_code, admin_areas: form.role_code === 'admin' ? form.admin_areas : [], account_status: form.account_status, reason: reason.trim() });
            setForm(updated); setReason(''); await onSaved();
    ...
    return <div className="account-access-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="account-access-panel" role="dialog" aria-modal="true">
        ...
        {loading || !form ? <div className="account-access-loading"><Loader2 className="spin" /> Loading access…</div> : <form onSubmit={submit}>
            ...
            <button className="account-access-save" disabled={saving || reason.trim().length < 3}>{saving ? <><Loader2 className="spin" size={16} /> Saving…</> : <><ShieldCheck size={16} /> Save access changes</>}</button>
        </form>}
    </section></div>;
};
