...
const planCards: PlanCard[] = [
  { code:'icad-foundations', name:'iCAD Foundations', shortName:'Foundations', level:1, outcome:'Master the fundamental layout, tools, and basic navigation of iCAD. By the end, you’ll be comfortable exploring the software and creating simple 3D primitives.', features:['Beginner-friendly tutorials','Basic UI navigation & origin','Direct Mentor Chat'] },
...
export const AccountCenterView: React.FC<{ page: Page }> = ({ page }) => {
  ...
  const [publicPlans, setPublicPlans] = useState<PublicAccessPlan[]>([]);
  ...
  useEffect(() => {
    let active = true;
    registrationService.getPlans().then(result => { if (active) setPublicPlans(result); }).catch(() => undefined);
    return () => { active = false; };
  }, []);

  /** Shows the published price, or the same contact line as the public landing page when no price is set. */
  const planPrice = (code: string) => {
    const published = publicPlans.find(item => item.code === code);
    if (!published || published.price_minor_units == null) return null;
    return new Intl.NumberFormat('en-US', { style: 'currency', currency: published.currency_code || 'USD', maximumFractionDigits: 0 })
      .format(published.price_minor_units / 100);
  };
  ...
  if (page === 'billing') return <div className="billing-modal-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)closeBilling();}}><section className="billing-modal billing-account-panel" role="dialog" aria-modal="true" aria-labelledby="billing-modal-title"><header className="billing-modal-header"><h1 id="billing-modal-title">Billing</h1><button onClick={closeBilling} aria-label="Close billing"><X size={21}/></button></header><div className="billing-modal-content">
    {selectedPlan&&<section className="billing-selected-plan"><div><span>{selectedAction==='switch'?'SELECTED PLAN CHANGE':'SELECTED UPGRADE'}</span><h2>{selectedPlan.name}</h2></div><div className="billing-selected-price">{planPrice(selectedPlan.code) ? <><strong>{planPrice(selectedPlan.code)}</strong><span>/user/month</span></> : <span>Pricing arranged with KMTI</span>}</div></section>}
