...
, nextLabel }) => {
  ...
  const LESSON_DATA: Record<string, { title: string; steps: string[] }> = {
    ...
    'standard-2': { title: 'GAS DISCHARGE, OIL GROOVE & SPROCKET', steps: [
      "Gas Discharge: To avoid deformation from heat during welding, add one ﾏ・ drill hole per square pipe for gas discharge.",
          ...
          {subLessonId === 'standard-2' && (
            <div className="tab-pane">
              <div className={`instruction-step ${currentIndex === 0 ? 'reading-active' : ''}`}>
                ...
                <p className="p">Deformation may happen due to the presence of heat and gas at time of welding. Holes added to square pipes for gas discharge. One <strong className="text-highlight">ﾏ・ Drill hole</strong> per square pipe is enough.</p>
                <div className="image-wrapper-flush">
            ...
            <div className="tab-pane">
              ...
              <div className={`instruction-step ${currentIndex === 1 ? 'reading-active' : ''}`}>
                ...
                <div className="info-box">
                  ...
                  <p className="p-flush">繧ｭ繝ｼ貅昴・豁ｯ螻ｱ驛ｨ荳ｭ蠢・↓蜷医ｏ縺帛刈蟾･縺吶ｋ縺薙→ (Key groove centered on tooth)</p>
                  <p className="p-flush">2. 譛ｬ蝗ｳ縺ｯ蟶りｲｩ蜩√・霑ｽ蜉蜉蟾･蝗ｳ縺ｧ縺ゅｋ (Purchased part with additional process)</p>
                </div>
              </div>
            </div>
          )}
          ...
          {subLessonId === 'standard-6' && (
            <div className="tab-pane">
              <div className={`instruction-step ${currentIndex === 0 ? 'reading-active' : ''}`}>
                ...
                <p className="p-flush"><strong>Example:</strong> (Bolt size ﾃ・1.5) + (Thickness) = (8 ﾃ・1.5) + 11 = 23mm 竕・25mm</p>
                <div className="info-box">
