...
export default function CurrentOriginIcon({ markOrigin = false, markXAxis = false, markYAxis = false, newOrigin = false, detailed = false }: { markOrigin?: boolean; markXAxis?: boolean; markYAxis?: boolean; newOrigin?: boolean; detailed?: boolean }) {
  const id = useId();
  if (markOrigin || markXAxis || markYAxis) {
    const active = markOrigin ? 0 : markXAxis ? 1 : 2;
    const points = [[153,480],[416,422],[344,54]];
    return <svg className="foundation-current-origin-icon" width="144" height="144" viewBox="50 25 470 550" aria-hidden="true">
    <defs>
    ...
    {points.slice(0, active + 1).map(([x,y],index)=><circle key={index} cx={x} cy={y} r="18" fill="#ff1616" stroke="#fff" strokeWidth="5" opacity={index===active ? 1 : 0.25} />)}
    <SelectionPointer x={points[active][0]} y={points[active][1]} />
  </svg>;
  }
  return <svg className="foundation-current-origin-icon" width="144" height="144" viewBox={detailed ? '55 80 390 390' : '50 65 430 460'} aria-hidden="true">
    <defs>
    ...
    <g fill={`url(#${id}-hole)`} stroke={detailed ? '#777f87' : 'none'} strokeWidth="2.5">
    ...
    <g strokeWidth={detailed ? 9 : 6} strokeLinejoin="round" transform={newOrigin ? 'translate(-80 45)' : undefined}>
    ...
    <path d="M144 342C96 314 111 247 155 215Q181 193 206 206L410 326 351 450Z" fill={`url(#${id}-body)`} stroke={detailed ? '#93999e' : 'none'} strokeWidth="2.5" />
    <ellipse cx="380" cy="391" rx="47" ry="71" transform="rotate(25 380 391)" fill="#e5e5e3" stroke={detailed ? '#777f87' : '#a5abb2'} strokeWidth={detailed ? 3 : 2} />
    ...
    {markYAxis && <g opacity="0.25" fill="#ff1616" stroke="#fff" strokeWidth="5"><circle cx="88" cy="438" r="18" /><circle cx="247" cy="345" r="18" /></g>}
    {markOrigin && <SelectionPointer x={88} y={438} />}
    {markYAxis && <SelectionPointer x={247} y={100} />}
  </svg>;
}
