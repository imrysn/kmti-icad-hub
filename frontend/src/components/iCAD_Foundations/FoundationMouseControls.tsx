...
function MouseDiagram({ control }: { control: number }) {
  const base = '#454b54';
  const accent = '#ed1745';
...
export default function FoundationMouseControls({ sections, pan = false, rotation }: { sections: Array<{title: string; text: string}>; pan?: boolean; rotation?: 'mouse' | 'alt' }) {
  const count = 4;
  return <div className="foundation-mouse-controls">
