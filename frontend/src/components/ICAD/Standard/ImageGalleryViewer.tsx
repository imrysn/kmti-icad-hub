...
const ImageGalleryViewer: React.FC<ImageGalleryViewerProps> = ({
  ...
  useEffect(() => { resetZoomPan(); }, [galleryIndex, resetZoomPan]);
    ...
    <div className="gallery-section-wrapper" style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
      {showCounter && (
        <div style={{ textAlign: "center", paddingBottom: "0.25rem", flexShrink: 0 }}>
          ...
          <h2 className="gallery-image-title" style={{ margin: 0, fontSize: "clamp(1.2rem, 3vw, 2rem)", fontWeight: 800, letterSpacing: "-0.02em", lineHeight: 1.15 }}>
            {currentImage.label}
          </h2>
        </div>
      )}
      ...
      <div
        ...
        style={{ position: "relative", width: "100%", maxWidth: "1000px", margin: "0 auto", minHeight: "320px", maxHeight: "72vh", height: "clamp(320px, 60vw, 640px)", flexShrink: 0, cursor: isPanningRef.current ? "grabbing" : "grab", userSelect: "none" }}
      >
        {/* Inner clipping container */}
        <div style={{ position: "absolute", inset: 0, background: "var(--gallery-bg, rgba(0,0,0,0.2))", borderRadius: "8px", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          <div style={{ transform: `translate(${pan.x}px, ${pan.y}px) scale(${zoom})`, transformOrigin: "center center", transition: isPanningRef.current ? "none" : "transform 0.05s ease-out", display: "flex", alignItems: "center", justifyContent: "center", width: "100%", height: "100%" }}>
            {currentImage.content ? currentImage.content : (
              <img src={currentImage.src} alt={currentImage.alt ?? String(currentImage.label)} draggable={false} loading="lazy" style={{ maxWidth: "100%", maxHeight: "100%", objectFit: "contain", pointerEvents: "none", transition: "opacity 0.25s ease" }} />
            )}
          </div>
        </div>
        ...
        >
          <div onPointerDown={handlePillPointerDown} title="Drag to move panel" style={{ cursor: "grab", padding: "8px", marginRight: "4px", borderRadius: "4px", display: "flex", alignItems: "center" }}>
          ...
          {isMulti && (
            <div style={{ position: "relative" }}>
              ...
              {showMenu && (
                <div ref={browseDropdownRef} style={{ position: "absolute", bottom: "calc(100% + 10px)", right: 0, width: "max-content", minWidth: "280px", maxWidth: "90vw", background: "var(--bg-surface, #0a0a12)", border: "1px solid rgba(221,77,250,0.4)", borderRadius: "14px", boxShadow: "var(--shadow-card, 0 24px 60px rgba(0,0,0,0.9))", zIndex: 1000, maxHeight: "440px", overflowY: "auto", padding: "0.5rem 0" }}>
                  <div style={{ padding: "0.65rem 1rem", fontSize: "0.65rem", fontWeight: 800, letterSpacing: "0.12em", textTransform: "uppercase", color: "#DD4DFA", borderBottom: "1px solid var(--border-color, rgba(255,255,255,0.07))", marginBottom: "0.25rem" }}>
                    {typeof images[galleryIndex].label === "string" ? images[galleryIndex].label as string : "Gallery"}
                  </div>
                  {images.map((img, idx) => (
                    <button
                      key={idx}
                    ...
                    >
                      <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: "22px", height: "22px", borderRadius: "6px", flexShrink: 0, background: idx === galleryIndex ? "rgba(221,77,250,0.3)" : "var(--bg-hover, rgba(255,255,255,0.07))", fontSize: "0.68rem", fontWeight: 800, color: idx === galleryIndex ? "#DD4DFA" : "var(--text-dim, rgba(255,255,255,0.4))" }}>
                        {img.number}
                      </span>
